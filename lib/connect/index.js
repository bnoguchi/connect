...
Server.prototype.use = function(route, handle){
    ...
    // Allows you to have access to the server when defining your handle.
    // This is important if we want to, e.g., add "upgrade" listeners to
    // the original connect Server via a handle that adds WebSocket support.
    if (handle.accessServer) handle.accessServer(this);
